...
extension Publishers.SwitchToLatest {
    fileprivate final class Outer<Downstream: Subscriber>
        : Subscriber,
          Subscription, // TODO: Test that it indeed conforms to Subscription
              ...
              Downstream.Failure == Upstream.Failure
    {
        ...
        private let _lock = UnfairLock.allocate() // 0x10
        private let downstreamLock = UnfairRecursiveLock.allocate() // 0x18
        private var _actualDownstream: Downstream?
        private var _upstreamSubscription: Subscription?
        private var _currentGenerationID = 0 // 0xA0
        private var _latest: InnerLatest? // 0xA8
        private var _demand = Subscribers.Demand.none // 0xB0
        private var _outerDone = false // 0xB8
        private var _innerDone = false // 0xC0
        ...
        private func innerReceive(_ input: NestedPublisher.Output) -> Subscribers.Demand {
            ...
            _demand -= 1 // TODO: Test that this crashes
            _lock.unlock()
            downstreamLock.lock()
...
extension Publishers.SwitchToLatest.Outer {
          ...
          CustomPlaygroundDisplayConvertible
    {
        ...
        fileprivate let _identifier: Int
        private var _innerSubscription: Subscription?

        init(parent: Parent, identifier: Int) {
            _parent = parent
